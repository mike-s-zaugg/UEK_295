...
export class TodoService {
  ...
  async create(corrId: number, userId: number, createTodoDto: CreateTodoDto): Promise<ReturnTodoDto> {
    this.logger.log(`${corrId} create called with userId: ${userId}, dto: ${JSON.stringify(createTodoDto)}`);
  ...
  async findAll(corrId: number, userId: number, isAdmin: boolean): Promise<ReturnTodoDto[]> {
    this.logger.log(`${corrId} findAll called with userId: ${userId}, isAdmin: ${isAdmin}`);
    let result;
    if (isAdmin) {
      result = await this.todoRepo.find() as any;
    } else {
      result = await this.todoRepo.find({ where: { createdById: userId } }) as any;
    }
  ...
  async findOne(corrId: number, id: number, userId: number, isAdmin: boolean): Promise<ReturnTodoDto> {
    this.logger.log(`${corrId} findOne called with id: ${id}, userId: ${userId}, isAdmin: ${isAdmin}`);
  ...
  async update(corrId: number, id: number, userId: number, isAdmin: boolean, updateDto: UpdateTodoDto): Promise<ReturnTodoDto> {
    this.logger.log(`${corrId} update called with id: ${id}, userId: ${userId}, isAdmin: ${isAdmin}, dto: ${JSON.stringify(updateDto)}`);
    const todo = await this.todoRepo.findOne({ where: { id } });
  ...
  async updateAdmin(corrId: number, id: number, dto: UpdateTodoAdminDto): Promise<ReturnTodoDto> {
    this.logger.log(`${corrId} updateAdmin called with id: ${id}, dto: ${JSON.stringify(dto)}`);
    const todo = await this.todoRepo.findOne({ where: { id } });
    if (!todo) throw new NotFoundException();
  ...
  async replace(corrId: number, id: number, dto: ReplaceTodoDto): Promise<ReturnTodoDto> {
    this.logger.log(`${corrId} replace called with id: ${id}, dto: ${JSON.stringify(dto)}`);
    const todo = await this.todoRepo.findOne({ where: { id } });
    if (!todo) throw new NotFoundException();
  ...
  async remove(corrId: number, id: number, isAdmin: boolean): Promise<ReturnTodoDto> {
    this.logger.log(`${corrId} remove called with id: ${id}, isAdmin: ${isAdmin}`);
    if (!isAdmin) {
    ...
    const result = await this.todoRepo.remove(todo) as any;
    this.logger.log(`${corrId} remove result: ${JSON.stringify(result)}`);
    return result;
  }
}