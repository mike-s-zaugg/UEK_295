...
/**
 * RepoMock is a type that represents a mocked version of a Repository for the ArticleEntity.
 * It is primarily used for testing purposes, allowing for mock implementations of key repository methods.
 *
 * RepoMock uses jest.Mocked to mock the selected methods of the Repository.
 * These methods include:
 * - `create`: Used to create new instances of ArticleEntity.
 * - `save`: Persists a given ArticleEntity or an array of ArticleEntities into the database.
 * - `find`: Retrieves an array of ArticleEntities matching the given criteria.
 * - `findOneBy`: Retrieves a single ArticleEntity by specified criteria.
 * - `remove`: Deletes a given ArticleEntity or an array of ArticleEntities from the database.
 *
 * This mocked type provides robust testing capabilities without the need for actual calls to the database.
 */
...
describe('ArticleService', () => {
  ...
  /**
   * Generates a base ArticleEntity object with default values.
   * The default values can be partially overridden by providing a partial object.
   *
   * @param {Partial<ArticleEntity>} [overrides={}] - Partial object containing properties to override the default values.
   * @returns {ArticleEntity} A complete ArticleEntity object combining default values and any provided overrides.
   */
  ...
  describe('create', () => {
    it('should create, set createdById/updatedById, save and return dto', async () => {
      ...
      // sicherstellen, dass Service die IDs setzt (mutiert "created")
